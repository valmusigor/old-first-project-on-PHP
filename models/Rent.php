...
class Rent {
    ...
    {
       return SQL::Instance()->select("SELECT * FROM rent"); 
    }
   ...
   {
       $offset=($page-1)*$count;
       return SQL::Instance()->select('SELECT * FROM rent WHERE status=\'2\' ORDER BY date DESC LIMIT '.$count.' OFFSET '.$offset);
   }
   public static  function geListCount()
   {

       return SQL::Instance()->select('SELECT COUNT(id) FROM rent WHERE status=\'2\'');
   }
   ...
   public static function checkShortContent($short_content){
    ...
    public static function addRent($short_content,$main_image,$path,$square,$price,$content,$userId,$type_tc,$address,$status){
        $path_images= serialize($path);
        if($main_image==0)
        $array=array('path_short_image'=>$path[0],'short_content'=>$short_content,'square'=>$square,'price'=>$price,'content'=>$content,'path_images'=>$path_images,'user_id'=>$userId,'type_tc'=>$type_tc,'address'=>$address,'status'=>$status);
        else
        $array=array('path_short_image'=>$path[$main_image],'short_content'=>$short_content,'square'=>$square,'price'=>$price,'content'=>$content,'path_images'=>$path_images,'user_id'=>$userId,'type_tc'=>$type_tc,'address'=>$address,'status'=>$status);    
        return SQL::Instance()->insert('rent',$array);
    }
    public static function editRent($id,$short_content,$main_image,$path,$square,$price,$content,$userId,$type_tc,$address,$status){
        $path_images= serialize($path);
        if($main_image==0)
        $array=array('path_short_image'=>$path[0],'short_content'=>$short_content,'square'=>$square,'price'=>$price,'content'=>$content,'path_images'=>$path_images,'user_id'=>$userId,'type_tc'=>$type_tc,'address'=>$address,'status'=>$status);
        else
